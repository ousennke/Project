...
// Helper to cache media items (fetch and create blob URL)
const cacheMediaItems = async (items: MediaItem[], corsProxy?: string): Promise<MediaItem[]> => {
    return Promise.all(items.map(async (item) => {
        // Only cache videos that are not already data URLs
        if (item.type === 'video' && !item.url.startsWith('data:')) {
            try {
                const fetchUrl = corsProxy ? `${corsProxy}${item.url}` : item.url;
                ...
                console.warn("Failed to cache video:", item.url, e);
            }
        }
        return item;
    }));
...
const AppContent: React.FC = () => {
    ...
    const addToHistory = async (status: 'success' | 'error', statusCode: number, body: any, duration: number, payload: any, errorMsg?: string) => {
        if (!selectedService) return;
        let media = extractMediaFromResponse(body);

        // Cache videos
        if (media.length > 0) {
            media = await cacheMediaItems(media, proxyUrl);
        }
    ...
    const handleSendRequest = async () => {
        ...
        try {
            ...
            setResponseData({
                ...
                headers: {},
                body: data || { error: "Failed to parse JSON response" },
                timestamp: duration,
                isPolling: false
            });

            if (!res.ok) {
                const errorBody = data ? JSON.stringify(data) : "";
                const errorMsg = data?.ResponseMetadata?.Error?.Message
                    || data?.message
                    || data?.error
                    || `HTTP Error ${res.status}: ${errorBody.substring(0, 200)}`;

                await addToHistory('error', res.status, data, duration, payload, errorMsg);
            ...
            if (selectedService.asyncConfig?.enabled) {
                ...
                if (!taskId) {
                    ...
                    await addToHistory('error', 200, data, duration, payload, msg); // Technical error despite 200 OK
                    throw new Error(msg);
                }
                ...
                const pollLoop = async () => {
                    if (Date.now() - pollStartTime > maxDuration) {
                        ...
                        await addToHistory('error', 408, { error: msg }, Date.now() - startTime, payload, msg);
                        return;
                    }
                    ...
                    try {
                        ...
                        if (status === config.pollSuccessValue) {
                            setLoading(false);
                            setResponseData(prev => prev ? { ...prev, isPolling: false } : null);
                            await addToHistory('success', pollRes.status, pollData, currentTotalDuration, payload);
                ...
                setLoading(false);
                // Sync success
                await addToHistory('success', res.status, data, duration, payload);
            }
